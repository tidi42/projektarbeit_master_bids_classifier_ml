#!/usr/bin/env Rscript
# =============================================================================
# create_gt_comparison_figures.R -- figures contrasting the two ground-truth runs
#   fixed      identity >= 0.90, coverage >= 0.80  (a priori)
#   calculated identity >= 0.92, coverage >= 0.50  (data-driven)
#
# Reads the tidy tables written by compare_gt_profiles.R and renders:
#   Figure_C1_ground_truth_definition   what changed in the labels, and why
#   Figure_C2_performance_concordance   model metrics under both definitions
#   Figure_C3_titration_and_ablation    dose-response + feature ablation
#   Figure_C4_hypothesis_agreement      effect sizes + verdict concordance
#
# Style matches create_project_figures.R (GraphPad/Prism, Okabe-Ito).
# Output: ./figures/comparison_fixed_vs_calculated/
# =============================================================================
...
p1a <- ggplot(sens, aes(covf, idf, fill = pct)) +
  ...
  geom_text(aes(label = sprintf("%.1f", pct), colour = txt), size = 2.8,
            fontface = "bold") +
  geom_tile(data = marks, fill = NA, colour = "black", linewidth = 1.4) +
  geom_text(data = marks, aes(label = tag, colour = txt), nudge_y = -0.29, size = 2.7,
            fontface = "bold") +
  scale_colour_identity() +
  scale_fill_viridis_c(option = "viridis", direction = -1,
                       breaks = c(85, 90, 95), guide = cbar(9)) +
  labs(title = "Cut-off sensitivity landscape",
       subtitle = "% of confident-Zymo reads retained; target >= 95%",
...
p1b <- ggplot(flip, aes(level, signed, fill = transition)) +
  ...
  labs(title = "Reads relabelled by the new cut-offs",
       subtitle = "2.82 % of all reads; 88 % of the churn sits at c1",
...
save_fig(figC1, "Figure_C1_ground_truth_definition", w = 13, h = 9)
...
p2a <- ggplot(cmb, aes(y = model_f)) +
  ...
  geom_point(aes(x = auprc_A, colour = A_LAB), size = 4.6) +
  geom_point(aes(x = auprc_B, colour = B_LAB), size = 2.6) +
...
p2b <- ggplot(fw, aes(delta, model_f)) +
  ...
  geom_point(size = 2.6, alpha = 0.75, colour = "#0072B2") +
  stat_summary(fun = median, geom = "point", shape = 124, size = 6, colour = "black") +
  labs(title = "Per-fold change in AUPRC",
       subtitle = "calculated - fixed; one point per donor fold, bar = median",
...
p2c <- ggplot(cw, aes(y = model_f)) +
  geom_segment(aes(x = A, xend = B, yend = model_f), colour = "grey55", linewidth = 1.1) +
  geom_point(aes(x = A, colour = A_LAB), size = 4.6) +
  geom_point(aes(x = B, colour = B_LAB), size = 2.6) +
  scale_colour_manual(values = pal_prof, breaks = PROF, name = "GT profile") +
...
save_fig(figC2, "Figure_C2_performance_concordance", w = 13, h = 9)
...
p3a <- ggplot(sel, aes(level, prec, colour = model_f, linetype = profile, group = interaction(model, profile))) +
  ...
  labs(title = "Precision vs titration under both definitions",
       subtitle = "Curves overlap: the dose-response is a property of the data, not the labels",
...
p3b <- ggplot(gap, aes(level, gap_xgb_minus_fixed, fill = profile)) +
  ...
  labs(title = "ML-over-baseline advantage (H3)",
       subtitle = "Grows ~10-13x from c1 to c5 under both definitions",
...
p3c <- ggplot(abl, aes(arm_f, fdr_pct, fill = profile)) +
  ...
  geom_errorbar(aes(ymin = pmax(fdr_pct - sd, 0), ymax = fdr_pct + sd),
                width = 0.2, linewidth = 0.6, position = position_dodge(0.78)) +
  scale_fill_manual(values = pal_prof, name = "GT profile") +
...
figC3 <- (p3a | p3b) / p3c +
  plot_annotation(tag_levels = "A",
    ...
    theme = theme(plot.title = element_text(face = "bold")))
save_fig(figC3, "Figure_C3_titration_and_ablation", w = 13, h = 9)
...
p4a <- ggplot(h, aes(y = idf)) +
  geom_vline(xintercept = 0, linetype = "dashed", colour = "grey45") +
  geom_segment(aes(x = effect_A, xend = effect_B, yend = idf), colour = "grey55", linewidth = 1.1) +
  geom_point(aes(x = effect_A, colour = A_LAB), size = 3) +
  geom_point(aes(x = effect_B, colour = B_LAB), size = 3) +
  scale_colour_manual(values = pal_prof, breaks = PROF, name = "GT profile") +
...
p4b <- ggplot(h, aes(abs(effect_A), abs(effect_B), colour = involves_baseline)) +
  ...
  geom_point(size = 3) +
  geom_text(aes(label = id), size = 2.7, vjust = -1.2, check_overlap = TRUE,
            show.legend = FALSE) +
...
p4c <- ggplot(h, aes(reorder(id, delta), delta, fill = grp)) +
  geom_hline(yintercept = 0, colour = "grey40", linewidth = 0.5) +
  geom_hline(yintercept = -base_gain, linetype = "dashed", colour = "#D55E00", linewidth = 0.7) +
  geom_col(width = 0.72) +
...
figC4 <- ((p4a | p4b) + plot_layout(widths = c(1.25, 1))) / p4c +
  plot_annotation(tag_levels = "A",
    ...
    theme = theme(plot.title = element_text(face = "bold")))
save_fig(figC4, "Figure_C4_hypothesis_agreement", w = 13, h = 9.5)

message("\nDone. Comparison figures written to: ", normalizePath(out_dir))
